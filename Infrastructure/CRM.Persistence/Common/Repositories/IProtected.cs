...
public interface IProtected<T> : IProtected where T : class
{
    ...
    IQueryable<T> Secured(int identityId, GroupRightsEnum requirement); // for Read
    ...
    Task<bool> HasAccess(T obj, int identityId, RepositoryOperationEnum operation, GroupRightsEnum requirement, CancellationToken cancellationToken); // for Write
}
